...
float kpR, kiR ; // best :0.01    0.001 
float kpL, kiL ; //best : 0.2 (max)    0.02

/*

for now :
 kpR =0.8, kiR = 0.005; // best :0.01    0.001 
...
float PWM_MIN_R_F = 80;//140;
float PWM_MAX_R_F = 180;//200;
float PWM_MIN_L_F = 140;//130;
float PWM_MAX_L_F = 255;//250;255


float PWM_MIN_R_B = -105;//140;
float PWM_MAX_R_B = -240;//200;
float PWM_MIN_L_B = -130;//130;
float PWM_MAX_L_B = -255;//250;255


float PWM_MIN_DOURA_R = 150;
float PWM_MAX_DOURA_R = 255;
float PWM_MIN_DOURA_L = 150;
float PWM_MAX_DOURA_L = 210;
...
moveDistance(505,500);
delay(1000);

moveDistance(-505,500);
delay(1000);
dour(150,300,0); // -180 = -164   +++ 180 = 150
*/
}
...
float constraint(float a, int min, int max) {
  if (a < min) a = min;
  if (a > max) a = max;
  return a;
}
...
// Separate PID constants
 kpL = 1.5, kiL = 0.1; //best : 0.2 (max)    0.02
 kpR =0.8, kiR = 0.05; // best :0.01    0.001 
...
void moveDistance(float distance, float speed) {
  ...
  while (abs(distance) < accel_dis + decel_dis) {
  ...
  while (abs(dS_total - distance) > 15) {
    ...
    if((abs(dS_total-distance) < accel_dis )){
      PWM_MAX_R_F=70;
      PWM_MIN_R_F=0;
        PWM_MAX_R_B=-105;
      PWM_MIN_R_B=0;
    ...
    if (((PWM_R >= PWM_MAX_R_F )||(PWM_R <= PWM_MAX_R_B ))&& (currentvelocityRight < targetSpeedRight)) {
    reachedLimitR = true;
    kiR = 0;
  }
  ...
  //      Serial.print(" ");
    ...
    if (((PWM_L >= PWM_MAX_L_F )||(PWM_L <= PWM_MAX_L_B ))&& (currentvelocityLeft < targetSpeedLeft)) {
      reachedLimitL = true;
      kiL = 0;
    }
...
//        Serial.print(" ");
    ...
    if (sens == 1) {
      PWM_R = constraint(PWM_R, PWM_MIN_R_F, PWM_MAX_R_F);
      PWM_L = constraint(PWM_L, PWM_MIN_L_F, PWM_MAX_L_F);
    } else {
      PWM_R = constraint(PWM_R, PWM_MAX_L_B, PWM_MIN_R_B);
      PWM_L = constraint(PWM_L, PWM_MAX_L_B, PWM_MIN_L_B);
    }
...
void dour(float angle, float speed, bool stop) {
  ...
  // Special case: small distance
  while (abs(distance) < accel + decel) {
    accel *= 0.8;
    decel *= 0.8;
  ...
  while (abs((theta * 180) / PI - angle) > 3) {
 ...
 float orientation_error = abs(totalR + totalL); 
    float angular_correction = kTheta * orientation_error;      
    float targetSpeedRight = sens * current_speed - angular_correction; // 
    float targetSpeedLeft  = sens * current_speed + angular_correction;
    ...
    if (!reachedLimitR) i_right_erreur += right_erreur;
    if (abs(PWM_R) >= PWM_MAX_DOURA_R && currentvelocityRight < targetSpeedRight) {
    reachedLimitR = true;
    kiR = 0;
  }
    PWM_R = kpR * right_erreur + kiR * i_right_erreur;
    ...
    PWM_R = (sens == 1) ? constraint(PWM_R, PWM_MIN_DOURA_R, PWM_MAX_DOURA_R) : constraint(PWM_R, -PWM_MAX_DOURA_R, -PWM_MIN_DOURA_R);

    // Left wheel PID
    left_erreur = -current_speed - currentvelocityLeft;
    if (!reachedLimitL) i_left_erreur += left_erreur;
    if (abs(PWM_L) >= PWM_MAX_DOURA_L && currentvelocityLeft < targetSpeedLeft) {
      reachedLimitL = true;
      kiL = 0;
    }
    PWM_L = kpL * left_erreur + kiL * i_left_erreur;
    ...
    PWM_L = (sens == 1) ? constraint(PWM_L, -PWM_MAX_DOURA_L, -PWM_MIN_DOURA_L) : constraint(PWM_L, PWM_MIN_DOURA_L, PWM_MAX_DOURA_L);
    ...
    PWM_R = (sens == 1) ? constraint(PWM_R, PWM_MIN_DOURA_R, PWM_MAX_DOURA_R) : constraint(PWM_R, -PWM_MAX_DOURA_R, -PWM_MIN_DOURA_R);
    PWM_L = (sens == 1) ? constraint(PWM_L, -PWM_MIN_DOURA_L, -PWM_MAX_DOURA_L) : constraint(PWM_L, PWM_MAX_DOURA_L, PWM_MIN_DOURA_L);
