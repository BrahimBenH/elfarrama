...
float kpR = 0.2, kiR = 0.1;
float kpL = 0.1, kiL = 0.05;
float kTheta = 2;
...
void moveDistance(float distance, float speed) {
  ...
  while (abs(dS_total - distance) > 5) {
    ...
    if (abs(PWM_R) >= PWM_MAX && current_speed < speed) {
      reachedLimitR = true;
      i_right_erreur = 0;
      kiR = 0;
    }

    if (sens == 1) PWM_R = erreur(PWM_R, PWM_MIN, PWM_MAX);
    else PWM_R = erreur(PWM_R, -PWM_MAX, -PWM_MIN);
    ...
    if (abs(PWM_L) >= PWM_MAX && current_speed < speed) {
      reachedLimitL = true;
      i_left_erreur = 0;
      kiL = 0;
    }

    if (sens == 1) PWM_L = erreur(PWM_L, PWM_MIN, PWM_MAX);
    else PWM_L = erreur(PWM_L, -PWM_MAX, -PWM_MIN);
